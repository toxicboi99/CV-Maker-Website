...
function initializeEventListeners() {
    // Only initialize CV builder listeners if we're on the CV page
    const personalDetailsForm = document.getElementById('personalDetailsForm');
    if (!personalDetailsForm) return; // Not on CV builder page

    // Step 1 Form
    personalDetailsForm.addEventListener('submit', handleStep1Submit);
    ...
    if (nextStep2) nextStep2.addEventListener('click', handleStep2Next);
    ...
    if (addWork) addWork.addEventListener('click', addWorkEntry);
    const addInterest = document.getElementById('addInterest');
    if (addInterest) addInterest.addEventListener('click', addInterestEntry);
    const addReference = document.getElementById('addReference');
    if (addReference) addReference.addEventListener('click', addReferenceEntry);
    const addSkill = document.getElementById('addSkill');
    if (addSkill) addSkill.addEventListener('click', addSkillEntry);
    const addLanguage = document.getElementById('addLanguage');
    if (addLanguage) addLanguage.addEventListener('click', addLanguageEntry);
    const addExtraSection = document.getElementById('addExtraSection');
    if (addExtraSection) addExtraSection.addEventListener('click', addExtraSection);
...
function updateProgressBar() {
    ...
    if (stepCircles.length === 0) return; // Not on CV builder page
